function search_env_for {
  ...
  cat <(ls -A ~/bin 2> /dev/null) <(ls -A /usr/local/bin 2> /dev/null) <(alias) <(env) <(declare -F) <(shopt) | grep -E -i "$search_regex";
}
...
function show_cmds_like {
  ...
  local alias=`echo "$search_res" | grep -E -i "\s*alias"`;
  [[ -z "$alias" ]] || { echo "$alias"; }
  local fn_names=$(echo "$search_res" | grep -E -i "\s*declare -f" | sed -E "s/declare -f (.*)/\1/" | xargs);
