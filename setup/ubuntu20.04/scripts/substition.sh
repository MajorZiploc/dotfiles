...
find "$tempShared" -regextype egrep -iregex ".*vim.*" -type f -exec sed -i'' 's,VIM_SHELL_PLACEHOLDER,/bin/zsh,g' {} \;
# THIS SUB APPEARS TO DO NOTHING. LOOK AT ~/.zshenv FOR WHAT THIS SHOULD DO
find "$tempShared" -regextype egrep -iregex ".*vim.*" -type f -exec sed -i'' 's,\.bashrc,.zshrc_core,g' {} \;
...
unset source_shellrc_placeholder
